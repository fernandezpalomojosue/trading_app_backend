...
class StrategyAIService:
    ...
    async def generate_strategy(
        ...
        """
        self.logger.info(
            "Starting AI strategy generation",
            component="strategy_ai_service",
            user_id=str(user_id),
        ...
        for attempt in range(1, max_attempts + 1):
            self.logger.info(
                "AI strategy generation attempt",
                component="strategy_ai_service",
                user_id=str(user_id),
                attempt=attempt,
                max_attempts=max_attempts
            ...
            try:
                ...
                if not dsl_definition:
                    self.logger.error(
                        "Missing dsl_definition field in AI response",
                        component="strategy_ai_service",
                        user_id=str(user_id),
                        attempt=attempt
                    )
                ...
                self.logger.debug(
                    ...
                    user_id=str(user_id),
                    attempt=attempt,
                    dsl_keys=list(dsl_definition.keys())
                )
