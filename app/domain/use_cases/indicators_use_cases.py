...
class IndicatorsUseCases(IndicatorsService):
    ...
    async def get_ema(
        ...
        # Transform to DTO
        results = [
            EMADataPoint(timestamp=item.get("timestamp", 0), value=item.get("value", 0.0))
    ...
    async def get_sma(
        ...
        # Transform to DTO
        results = [
            SMADataPoint(timestamp=item.get("timestamp", 0), value=item.get("value", 0.0))
            for item in raw_data
        ]
    ...
    async def get_rsi(
        ...
        # Transform to DTO
        results = [
            RSIDataPoint(timestamp=item.get("timestamp", 0), value=item.get("value", 0.0))
            for item in raw_data
        ]
